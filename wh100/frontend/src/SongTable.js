              ...
              <tbody className="divide-y divide-gray-300">
                {songs.map((song, index) => (
                  <tr key={song.id} className="bg-white transition-all duration-500 hover:bg-gray-50">
                    <td className="px-4 w-12 whitespace-nowrap text-center leading-6 font-medium text-gray-900 gap-3">{index + 1}</td>

                    <td class=" px-0 py-2">
                      <div class="flex items-center gap-3 w-80 break-text">
                          <img src={song.image_url} className="h-14 w-14 rounded-md object-cover" alt={song.name} />
                          <div class="data">
                              <p class="font-normal text-base leading-[1.2] text-gray-900 line-clamp-2">{song.name}</p>
                              <p class="font-normal text-sm leading-5 text-gray-400 line-clamp-1"> {song.artist_names.join(", ")} </p>
                          </div>
                      </div>
                    </td>
                    <td class="py-4 w-24 flex items-center justify-center">
                        <button onClick={() => handleDelete(song)} class="flex p-2 rounded-full bg-white group transition-all duration-500 hover:bg-red-600">
                            <svg className="flex justify-center w-5 h-5 items-center" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
